...
docker run --rm -v ${PWD}/$BUILD_DIR:/documents asciidoctor/docker-asciidoctor:1.2.0 asciidoctor-revealjs -r asciidoctor-diagram -a icons=font -a revealjs_theme=white -a source-highlighter=rouge -a imagesdir=images -a revealjsdir=revealjs -a revealjs_slideNumber=c/t -a revealjs_transition=slide -a revealjs_hash=true -a sourcedir=src/main/java -b revealjs '*.adoc'
