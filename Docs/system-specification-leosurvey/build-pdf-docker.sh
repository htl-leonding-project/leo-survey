...
docker run --rm -v ${PWD}/$BUILD_DIR:/documents asciidoctor/docker-asciidoctor asciidoctor-pdf -r asciidoctor-diagram -a icons=font -a icons-set=fas -a rouge-style=github -a experimental=true -a source-highlighter=rouge -a imagesdir=images -a toc=left -a toclevels=2 -a sectnums=true -a sourcedir=src/main/java -b pdf '*.adoc'
